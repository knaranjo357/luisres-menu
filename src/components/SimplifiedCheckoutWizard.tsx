...
import { saveCustomerInfo, loadCustomerInfo } from '../utils/customerCache';
...
const SimplifiedCheckoutWizard: React.FC<SimplifiedCheckoutWizardProps> = ({
  ...
  useEffect(() => {
    const cachedInfo = loadCustomerInfo();
    if (cachedInfo) {
      setCustomerInfo(prev => ({
        ...prev,
        name: cachedInfo.name || prev.name,
        phone: cachedInfo.phone || prev.phone,
        address: cachedInfo.address || prev.address,
        city: cachedInfo.city || prev.city,
        paymentMethod: cachedInfo.paymentMethod || prev.paymentMethod,
        deliveryType: cachedInfo.deliveryType || prev.deliveryType,
      }));
      
      if (cachedInfo.city) {
        setSelectedCity(cachedInfo.city);
      }
      if (cachedInfo.neighborhood) {
        setSelectedNeighborhood(cachedInfo.neighborhood);
      }
    }
  }, [setCustomerInfo]);
  ...
  }, [customerInfo, selectedCity, selectedNeighborhood]);
  ...
  const canProceedToNext = () => {
    switch (currentStep) {
      ...
      case 3: // Address (only for delivery)
        ...
        return customerInfo.address.trim() && selectedCity && selectedNeighborhood;
  ...
  const handleSubmit = () => {
    // Guardar datos finales en caché antes de enviar
    saveCustomerInfo({
      ...customerInfo,
      city: selectedCity,
      neighborhood: selectedNeighborhood,
          ...
          {currentStep === 3 && customerInfo.deliveryType === 'delivery' && (
            ...
            >
              ...
              <div className="space-y-4">
                {/* City Selection */}
                <div>
                  <label className="block text-wood-dark font-medium mb-2">Ciudad</label>
              ...
              {customerInfo.paymentMethod === 'transferencia' && (
                ...
                >
                  <p className="text-blue-700 font-medium text-center mb-2">
                    📱 Número: {restaurantConfig.bankAccount || '300 123 4567'}
                  </p>
